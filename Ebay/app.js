...
http.createServer(app).listen(app.get('port'), function(){
	...
	mysql.executeSQL(Constants.SELECT_PRODUCT_QUERY, function(err, result) {
		console.log("Cache in 1");
		Redis.cacheProducts(result);
	});
	mysql.executeSQL(Constants.SELECT_CAT_QUERY, function(err, result) {
		console.log("Cache in 2");
		Redis.cacheCategories(result);
	});
	mysql.executeSQL(Constants.SELECT_SELLER_QUERY, function(err, result) {
		console.log("Cache in 3");
		Redis.cacheSellers(result);
	});
	mysql.executeSQL(Constants.SELECT_COUNT_QUERY, function(err, result) {
	console.log("Cache in 4");
		Redis.cacheCount(result);
	});
	mysql.executeSQL(updateBid, function(err, result) {
